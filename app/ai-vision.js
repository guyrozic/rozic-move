// Faithful port of Hovalot's src/services/aiVisionCore.ts — the item-catalog photo-scan
// path (analyzeImagesWithCatalog), the free-text "chat to add items" path
// (chatAddItemsWithCatalog/buildChatPrompt), the support-chat agent (chatSupportAgent),
// AND the marketplace listing AI (analyzeListingPhoto from aiVisionCore.ts + askListingAI
// from AIChatHelper.tsx).
//
...
// Verbatim from aiVisionCore.ts's buildAnalysisPrompt — do not paraphrase when editing,
// this wording has been tuned against real photos. ⚠️ The block order (catalog+rules
// first, roomLabel at the very end) is deliberate — see the prefix-cache note on
// buildChatPrompt below, which applies here identically (17.9 sync).
...
You were given one or more photos of a single room. That room's label appears at the very END of this prompt, under "Room".

The catalog below spans EVERY category in the entire app (not just that room), because people sometimes photograph items that don't "belong" to the expected room (e.g. a treadmill photographed in a bedroom, or a box of kitchen items stored in a hallway). Match every visible object to its closest catalog entry regardless of which room it would normally belong to — the room label is context, not a restriction.
...
3. If you see an object clearly but are unsure exactly which catalog variant fits, DO NOT skip it — add a clarification question to "questions". There are two distinct kinds of ambiguity, and they must be phrased differently:
   ...
   c. COUNT BEFORE YOU ASK — this applies to BOTH 3a and 3b, and it is the single most expensive thing to get wrong. You already count correctly for rule 2 items (e.g. you will correctly return office_chair×2 or dining_chair×4). Apply the SAME counting discipline here: an object that goes through a size/type question does NOT become one unit just because you are asking about it. SCAN the photo and count the physical units FIRST, before you phrase anything. If you see two beds, two desks, two wardrobes — that is qty 2, even when the two units look identical and even when a single question covers both. State the count inside the question text (e.g. "(2 יחידות)") and set EVERY option's "qty" to the counted number, never to a default of 1.
...
Now analyze the given photos for that room and respond with JSON only, in the exact shape shown above — no extra text and no markdown.`;
}
...
// זהה ל-parseItemsAndQuestions ב-aiVisionCore.ts, כולל הוספת אוטומטית של
// "ביטול" ו-"אחר — כתוב בצ'אט" לכל שאלה (17.9 — ה"אחר" היה חסר: שאלות
// גודל/סוג באתר לא הציעו למשתמש דרך-מילוט כשאף אפשרות לא התאימה).
  ...
  const questions = Array.isArray(parsed.questions)
    ? parsed.questions
        ...
        .map((q) => {
          ...
          const withChat = hasChat ? withCancel : [...withCancel, { label: 'אחר — כתוב בצ\'אט', itemKey: null, qty: 0, isChat: true }];
...
/** Analyzes photos against a full {key,label,synonyms} catalog — mirrors analyzeImagesWithCatalog in aiVisionCore.ts. images: [{base64, mimeType}]. */
...
// Verbatim from aiVisionCore.ts's buildChatPrompt — do not paraphrase when editing,
// this wording has been tuned against real conversations.
//
// ⚠️ 15.9 (Hovalot) — **הסדר של הבלוקים כאן נקבע ע"י מטמון ה-prefix של
// Gemini, לא ע"י קריאוּת, ואסור "לתקן" אותו בחזרה.** הקטלוג + כללי המשימה
// הם תחילית משותפת ויציבה לכל קריאה (~25,000 תווים) — roomLabel/רשימת
// הפריטים/ההיסטוריה/ההודעה החדשה (המשתנים בכל תור, ולעיתים גם ב-retry על
// אותה הודעה) יושבים **מתחת**, כדי שהתחילית המשותפת תיפגע במטמון המרומז
// של gemini-flash-latest (75% הנחה) על תחילית זהה בבתים. הועתק מילה-במילה
// כשההעתקה הזו סונכרנה (17.9).
...
The room/context the user is in, the items already in their list, the recent conversation, and the NEW MESSAGE you need to act on all appear at the very END of this prompt, after the task rules. The catalog below spans EVERY category in the entire app (not just that room), because people sometimes have items that don't belong to the "expected" room (e.g. a bed stored in the kitchen). Always search the FULL catalog, not just items that "belong" to that room.
...
6. If there are several distinct possible catalog matches and you genuinely cannot decide between them, ask a clarification question in "questions" — but the way you phrase it depends on WHAT kind of ambiguity it is:
   ...
   Either way, list every relevant option — not an arbitrary subset. A short, generic, single-word message like "ארון" or "שולחן" is normal and expected to hit this rule — treat it as a real request to classify via a question, never as something you failed to understand.
6c. Whenever ANY "questions" entry is included in your response (rules 4, 5, 6, 6a, 6b, 7, 8, 9a — not just this one), do NOT also restate that same question in "reply" (no duplicating the question as both prose and buttons — the UI already shows the question text itself once, right above its options, and separately echoes it into the chat log the moment the user answers, so a third copy in "reply" reads as the same thing repeated). Leave "reply" empty, or use it only for something genuinely separate from the question being asked — e.g. rule 9's freeItems clarification about a DIFFERENT, unrelated item in the same message.
...
9. Use "freeItems" ONLY as a last resort — when the item is truly unique and has absolutely no catalog equivalent (e.g. a custom-built piece, a very specific machine, a pet). Common household/office items should ALWAYS match something in the catalog. Before adding anything to "freeItems", also check that it actually reads like a real physical item someone would hire movers for. If the wording is unclear, looks like a typo, or is a strange/unexpected word with no context that doesn't obviously belong in a house move (e.g. a food item, a single odd word) — do NOT add it to "freeItems" yet. Instead, leave "freeItems" empty for it this turn and, in "reply" only, ask the user to clarify what they meant (quote their exact word back to them) — wait for their next message before deciding whether it's a genuine item to add, a typo for something else, or not relevant at all. Don't put this kind of clarification in "questions" — there's no clean set of button options to offer here, a plain follow-up question in "reply" is the right tool.
9a. If the item you're about to add — to "items" OR "freeItems" — would end up with an implausibly large quantity for what it physically is (e.g. hundreds or thousands of a small household object like a salt shaker, a fork, a sock, a single small toy), this is almost always because the customer actually has many small/miscellaneous items packed together in cardboard boxes, not that many literal loose units to load one by one. Do NOT add it at that face-value quantity. Instead, ask a clarification question in "questions": confirm they're packed in boxes and let them pick a box size — build the options from whichever box_* itemKeys exist in the catalog above (use their real labels, e.g. "ארגז קרטון S"), each option adding a reasonable starter quantity of that size (e.g. qty 3, since the user can always adjust the exact number afterward in their item list) — and mention in "reply" that they can tweak the box count once added. Leave the original oversized item out of "items"/"freeItems" entirely this turn; wait for the box answer instead of guessing both at once.
...
FINAL CHECK before you respond — did this message just cause a bed (any size/type) to be added to "items"? If yes, "questions" MUST include the mattress follow-up from rule 7, even if the bed was added as the answer to a previous size question, or alongside other unrelated items in a busy message. Never skip this just because the message was about several things at once.
...
Current room/context: "${roomLabel}"
...
Now apply the task rules above to that NEW MESSAGE and respond with JSON only, in the exact shape shown above — no extra text and no markdown.`;
}
...
// זהה ל-IMPLAUSIBLE_FREE_ITEM_QTY / sanitizeFreeItemQuantities ב-aiVisionCore.ts
// (17.9 — היה חסר לגמרי באתר). רשת ביטחון דטרמיניסטית מעל כלל 9a בפרומפט:
// גם אם המודל מתעלם מהכלל (ציות לפרומפט אינו מובטח), שום freeItem בכמות
// דמיונית (למשל "מלחייה × 5000") לא מגיע ללקוח כמות שהיא — הוא נשלף
// ומוחלף בשאלת-כפתור אמיתית שמשתמשת בפריטי הארגזים מהקטלוג.
const IMPLAUSIBLE_FREE_ITEM_QTY = 20;

function sanitizeFreeItemQuantities(freeItems, questions) {
  const safeFreeItems = [];
  const extraQuestions = [];
  freeItems.forEach((fi, idx) => {
    if (fi.action !== 'remove' && fi.qty >= IMPLAUSIBLE_FREE_ITEM_QTY) {
      extraQuestions.push({
        id: `qty-confirm-${Date.now()}-${idx}`,
        text: `רשמת "${fi.label}" בכמות ${fi.qty} — כנראה שהכוונה למספר ארגזים ולא ${fi.qty} יחידות בודדות. איזה ארגז תרצה להוסיף?`,
        options: [
          { label: 'ארגז קרטון קטן (S, עד 30 ס"מ לצד)', itemKey: 'box_S', qty: 3 },
          { label: 'ארגז קרטון בינוני (M, 30–40 ס"מ לצד)', itemKey: 'box_M', qty: 3 },
          { label: 'ארגז קרטון גדול (L, 40–50 ס"מ לצד)', itemKey: 'box_L', qty: 2 },
          { label: `לא, באמת ${fi.qty} יחידות בודדות`, itemKey: null, qty: 0, isChat: true },
          { label: 'ביטול — לא רלוונטי', itemKey: null, qty: 0 },
        ],
      });
    } else {
      safeFreeItems.push(fi);
    }
  });
  return { freeItems: safeFreeItems, questions: [...questions, ...extraQuestions] };
}

/**
 * Free-text "add items by chat" — mirrors chatAddItemsWithCatalog in aiVisionCore.ts.
 ...
 * all empty AND reply is also empty) — not for HTTP/JSON errors, those just throw.
 * ⚠️ 17.9 — the retry gate now also checks `!hasReply` (matching
 * callGeminiChatWithRetry in aiVisionCore.ts): rule 9's ambiguous-freeItem
 * clarification deliberately returns this same "everything empty except reply"
 * shape, and that's a real answer, not a dead response — it must not be discarded.
 */
...
/** Analyzes listing photos into a draft — mirrors analyzeListingPhoto in aiVisionCore.ts. images: [{base64, mimeType}]. */
...
/* ══════════════════════════════════════════════════════════════════
   ...
   matches this speaker"). 17.9 — נבדק שוב מול aiVisionCore.ts: הענפים
   הלקוחיים (1,2,3,4,5,6,6a,6b) תואמים מילה-במילה חוץ מהתאמות אתר/אפליקציה
   מכוונות ("באפליקציה"→"באתר" וכו'), וזה בדיוק מה שאמור לקרות כאן — לא נגעתי.
   ══════════════════════════════════════════════════════════════════ */
...
export function friendlyAIError(err, notConfiguredMessage) {
  ...
  /**
   * ⚠️ 20.9 — `geminiProxy.ts` דוחה בקשה עם 413 בשני מקרים: `too_many_parts`
   * (מספר התמונות + חלק הטקסט עולה על `MAX_PARTS`=12, כלומר עד 11 תמונות)
   * ו-`payload_too_large` (הבקשה ב-base64 עולה על 8MB, גם במספר תמונות
   * קטן מ-11 אם הן בקבצים גדולים). עד היום שני המקרים נפלו לברירת המחדל —
   * "בדוק את החיבור לאינטרנט" — והאשימו את הרשת של הלקוח בכמות/גודל
   * התמונות שהוא בחר לנתח בבת אחת. `callGemini` שולחת את הסטטוס בתוך
   * `AI_REQUEST_FAILED: ${status} ${errText}`, ולכן הבדיקה על התבנית הזו.
   */
  if (msg.includes('AI_REQUEST_FAILED: 413')) {
    return 'יותר מדי תמונות בבת אחת, או שהן גדולות מדי לניתוח יחד. נתח פחות תמונות בכל פעם, ואת השאר בסריקה נוספת.';
  }
